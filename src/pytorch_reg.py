...
class CSVDataset(Dataset):
    ...
    def __init__(self, path):
        ...
        self.X = prepro.fit_transform(self.X)

        # self.X = df.drop(['Oxygen'], axis=1).astype('float32')
        self.y = df.values[:, -1].astype('float32')
        # self.y = prepro.fit_transform(self.y.reshape(-1, 1))
...
# prepare the data
path = '../V02Max/data/concatedData/combinedHeaderlessDropped.csv'
...
row = prepro.transform([[131.3,90,137.85,25.7]])
